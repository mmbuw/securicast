...
public class AccountDb {
  ...
  private static final String WBT_SECRET_COLUMN = "wbtsecret";
  // @VisibleForTesting
  ...
  public AccountDb(Context context) {
    ...
    mDatabase.execSQL(String.format(
        ...
        " %s INTEGER DEFAULT %s, %s INTEGER, %s INTEGER DEFAULT %s, %s TEXT)",
        TABLE_NAME, ID_COLUMN, EMAIL_COLUMN, SECRET_COLUMN, COUNTER_COLUMN,
        DEFAULT_HOTP_COUNTER, TYPE_COLUMN,
        PROVIDER_COLUMN, PROVIDER_UNKNOWN, WBT_SECRET_COLUMN));
  ...
  public String getWebBluetoothSecret(String email) {
    Cursor cursor = getAccount(email);
    try {
      if (!cursorIsEmpty(cursor)) {
        cursor.moveToFirst();
        return cursor.getString(cursor.getColumnIndex(WBT_SECRET_COLUMN));
      }
    } finally {
      tryCloseCursor(cursor);
    }
  ...
  /**
   * Save both keys to database, for otp generation and bluetooth data encryption,
   * creating a new user entry if necessary.
  ...
  public void update(String email, String secret, String oldEmail,
                     OtpType type, String wBTSecret) {
    update(email, secret, oldEmail, type, wBTSecret, null);
  }
   ...
   */
  public void update(String email, String secret, String oldEmail,
                     OtpType type, String wBTSecret, Boolean googleAccount) {
    ContentValues values = new ContentValues();
    values.put(EMAIL_COLUMN, email);
    values.put(SECRET_COLUMN, secret);
    values.put(TYPE_COLUMN, type.ordinal());
    values.put(WBT_SECRET_COLUMN, wBTSecret);
